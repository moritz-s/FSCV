...
with nidaqmx.Task() as task_in:
    ...
    task_in.ai_channels.add_ai_voltage_chan("/PXI1Slot4_2/ai0")

    task_in.timing.ref_clk_src = "PXIe_Clk100"
    task_in.timing.ref_clk_rate = 100000000
    task_in.timing.cfg_samp_clk_timing(
        1000, sample_mode=AcquisitionType.CONTINUOUS)
    task_in.triggers.sync_type.MASTER = True

    slave_task.triggers.start_trigger.cfg_dig_edge_start_trig(
        "/PXI1Slot4_2/ai/StartTrigger")

    print('2 Channels 1 Sample Read Loop 10: ')
    task_in.start()

    for _ in range(10):
        master_data = task_in.read(number_of_samples_per_channel=10)
